...
class Contract < ThorBase
  ...
  CONTRACT_FIELDS = "{id uuid}"

  desc "list", "list contracts"
  def list
    expression = "{contracts #{CONTRACT_FIELDS}}"
    puts graphql_client.query(expression).to_h
  end

  desc "show ID", "show contract information"
  def show(id)
    expression = "{contract(id: #{id}) #{CONTRACT_FIELDS}}"
    puts graphql_client.query(expression).to_h
  end
