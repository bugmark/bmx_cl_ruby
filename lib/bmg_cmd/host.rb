...
class Host < ThorBase
  ...
  def info
    expression = "{host {info {hostName hostTime dayOffset hourOffset}}}"
    puts graphql_client.query(expression).to_h
  end

  desc "count", "show current host counts"
  def count
    expression = "{host {count {users trackers issues offers contracts}}}"
    puts graphql_client.query(expression).to_h
  end

  desc "reset", "reset system"
  def reset
    expression = "{host_reset}"
    puts graphql_client.mutation(expression).to_h
  end

  desc "seed", "generate seed data"
  def seed
    expression = "{host_seed}"
    puts graphql_client.mutation(expression).to_h
  end
