class Offer < ThorBase
  ...
  OFFER_FIELDS = "{id stmIssueUuid stmStatus stmTitle type uuid}"

  desc "list", "list offers"
  def list
    expression = "{offers #{OFFER_FIELDS}}"
    puts graphql_client.query(expression).to_h
  end

  desc "show ID", "show offer information"
  def show(id)
    expression = "{offer(id: #{id}) #{OFFER_FIELDS}}"
    puts graphql_client.query(expression).to_h
  end
