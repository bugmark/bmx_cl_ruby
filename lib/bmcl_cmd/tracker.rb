...
class Tracker < ThorBase
  ...
  TRACKER_FIELDS = "{id uuid}"

  desc "list", "list trackers"
  def list
    expression = "{trackers #{TRACKER_FIELDS}}"
    puts graphql_client.query(expression).to_h
  end

  desc "show ID", "show tracker information"
  def show(id)
    expression = "{tracker(id: #{id}) #{TRACKER_FIELDS}}"
    puts graphql_client.query(expression).to_h
  end
