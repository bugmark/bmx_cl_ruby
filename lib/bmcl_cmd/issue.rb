class Issue < ThorBase
  ...
  def list
    expression = "{issues {id sequence stmIssueUuid stmStatus stmTitle uuid}}"
    puts graphql_client.query(expression).to_h
  end

  desc "show ID", "show issue information"
  def show(id)
    expression = "{issue(id: #{id}) {id sequence stmIssueUuid stmStatus stmTitle uuid}}"
    puts graphql_client.query(expression).to_h
  end
